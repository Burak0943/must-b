...
const Login = () => {
  ...
  // nextUrl mantığını koruyoruz, OAuth için de bunu kullanacağız
  const nextUrl = searchParams.get("next") ?? "/dashboard";

  // YÖNLENDİRME BEYNİ (Asılı kalmayı ve döngüyü çözen kısım)
  useEffect(() => {
    let mounted = true;
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      // Sadece başarılı girişte ve bileşen ekrandayken fırlat
      ...
      if (signInError) {
        if (signInError.message.includes("Invalid login credentials")) {
          ...
          if (signUpData.user && !signUpData.session) {
            ...
            setLoading(false); // E-posta doğrulaması gerektiği için yükleniyor ikonunu kapatıyoruz
          }
        } else {
          throw signInError;
        }
      }
      // NOT: Giriş başarılı olursa, useEffect içindeki onAuthStateChange anında navigate edecek.
    } catch (error: any) {
      toast.error(error.message || "Error occurred.");
      setLoading(false); // Sadece hata anında butonu geri getir
    }
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen flex items-center justify-center p-6 relative">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="w-full max-w-md p-8 glass rounded-outer relative z-10">
        ...
        <div className="space-y-4">
          {/* 2 Sütundan 3 Sütuna Çıkarıldı (Apple eklendi) */}
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="relative group">
              ...
              <motion.button
                ...
                type="button"
                className="relative w-full py-3 px-4 glass rounded-xl flex items-center justify-center gap-2 hover:bg-white/10 transition-all font-medium text-foreground text-sm border border-white/5 overflow-hidden"
              >
                <motion.div
                  animate={{ x: ["-100%", "200%"] }}
            ...
            <div className="relative group">
              ...
              >
                <motion.div
                  ...
                  className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -skew-x-12"
                />
                <Apple className="w-4 h-4 relative z-10" />
